        ...
        <view class="car-card">
          ...
          <view class="addr">
            ...
            <text class="addr-text">
              {{ carInfo.netPointName || '晋江市｜溜溪花园（停车勿超过石墩勿停收费处，靠右侧树边停车，左边过道不能停，否则后果自负）' }}
            </text>
...
<view class="card pkg-card">
  ...
  <!-- 未选择：黄条 + 右侧深灰胶囊按钮 -->
  <view
    v-if="!selectedCombo?.comboId"
    class="pkg-empty"
    @tap="navigateToComboList"
  >
   <text class="pkg-tip">购买套餐更划算哦~</text>
    <view class="pkg-btn">
      <text class="pkg-btn-text">去购买</text>
      <text class="pkg-btn-arrow">›</text>
    </view>
  </view>

  <!-- 已选择：沿用你原先的展示 -->
  <view v-else class="pkg-inner" @tap="navigateToComboList">
    <view class="pkg-left">
      <text class="pkg-title">{{ selectedCombo?.remarks || '三小时免里程套餐' }}</text>
      <text class="pkg-sub" :class="{ red: !!selectedCombo?.comboId }">
        {{ selectedCombo?.comboId ? (comboSaving > 0 ? `已为您节省${fmt(comboSaving)}元！` : '套餐已选择') : '套餐未选择' }}
      </text>
    </view>
    <view class="pkg-cta">
...
<view class="coupon-card">
  <view class="coupon-hd">优惠券</view>

  <!-- 未选择：整条橙色渐变按钮 -->
  <view
    v-if="!selectedCoupon?.couponId"
    class="coupon-empty"
    @tap="navigateToCouponList"
  >
    <text class="coupon-empty-text">
      {{ (couponList && couponList.length>0)
          ? `${couponList.length}张优惠券可用，快去选择吧~`
          : '暂无可用优惠券，看看活动~' }}
    </text>
    <text class="coupon-empty-arrow">›</text>
  </view>

  <!-- 已选择：沿用你原来的逻辑 -->
  <view v-else class="coupon-inner" @tap="navigateToCouponList">
   ...
   <view class="agree-line" @tap="openAgreementFlow">
     ...
     <text class="agree-text">
      ...
      <view class="foot-bar">
        <view class="sum">
          ...
          <view class="sum-val">
            <text class="yen">¥</text>
            <text class="n din68">{{ finalAmount }}</text>
    ...
    <ControlModal
	  ...
	  @change-location="chooseReturnPoint"
    ...
    <rule-popup ref="rulePopup" @confirm="ConfirmOK" />
	 ...
	 <!-- 或者直接不传，默认就是 2/3 -->
	  ...
	  <!-- 取消 = 返回上一页 -->
	   <!-- 背后不可操作，不给点遮罩关闭 -->
	<CarPhotoUploadSheet
	  :visible="showPickupPhoto"
	   snap="custom"        
	  :initialRatio="0.822"     
	  mode="pickup"
	  cancelAction="close"       
	  :closeOnMask="false"     
	  @submit="onPickupSubmit"
	  @close="showPickupPhoto=false"
	/>
	<CarPhotoUploadSheet
	  :visible="showReturnPhoto"
	  snap="custom"
	  :initialRatio="0.888"  
	  mode="return"
	  cancelAction="close"
...
import CarPhotoUploadSheet from '../../components/CarPhotoUploadSheet.vue'

import { safeNavigateTo, safeRedirectTo, isNavBusy } from '@/utils/navGuard'

// import { openCar, closeCar, hornCar } from '../../api/index.js'
import { preloadCarInfo, createOrder, settleOrder, payOrder } from '../../api/order.js'
...
const isRouting = ref(false) // 路由中/防重复点击
...
const now = new Date()
tripStartTime.value = now.toISOString()

/* ========= 取车拍照 ========= */
const showPickupPhoto = ref(false)
const hasFirstOpenProof = ref(false)
const ORDER_OPEN_PROOF_KEY = computed(() => {
  const n = String(uni.getStorageSync('currentOrderNum') || '')
  return n ? `order:${n}:first-open-proof` : ''
})

/* ========= 还车拍照 ========= */
const showReturnPhoto = ref(false)
const hasReturnProof = ref(false)
const RETURN_PROOF_KEY = computed(() => {
  const n = String(uni.getStorageSync('currentOrderNum') || '')
  return n ? `order:${n}:return-proof` : ''
})

onMounted(() => {
  if (RETURN_PROOF_KEY.value) hasReturnProof.value = !!uni.getStorageSync(RETURN_PROOF_KEY.value)
...
// 选择网点（安全跳转）
async function chooseReturnPoint () {
  if (!carId.value) { uni.showToast({ title:'缺少车辆ID', icon:'none' }); return }
  if (isRouting.value || isNavBusy()) return
  isRouting.value = true
  try {
    uni.setStorageSync('order:currentCarId', Number(carId.value))
    await safeNavigateTo(`/pkg-biz/pages/find/return?carId=${Number(carId.value)}`)
  } finally {
    setTimeout(() => { isRouting.value = false }, 300)
  }
}

watch(carId, (nv, ov) => {
  if (ov && nv !== ov) {
    bindEmptyForCurrentCar()
    // 清空套餐和优惠券
    selectedCombo.value = {}
...
const controlModalLocation = computed(() => {
  return returnPoint.value?.name || carInfo.value?.netPointName || ''
})
...
const finalAmount = computed(() => {
  const base = Number(bottomAmount.value || 0);
  const discount = selectedCoupon.value.discountAmount || 0;
  const comboSavingV = selectedCombo.value.saveAmount || 0;
  return fmt(base - discount - comboSavingV);
})
...
onLoad(async (options) => {
  ...
  if (carId.value && carId.value === lastCarId) {
...
async function navigateToCouponList(){
  ...
  isRouting.value = true
  try {
    await safeNavigateTo(buildCouponNavigateParams({ carId: carId.value, isPayment: isPayment.value, defaultTab: 'coupon' }))
  } finally { setTimeout(() => { isRouting.value = false }, 300) }
}

async function navigateToComboList(){
  uni.setStorageSync('tempPackage', selectedPackage.value)
  uni.setStorageSync('tempCombo', selectedCombo.value)
  uni.setStorageSync('tempIsPayment', isPayment.value)
  uni.setStorageSync('tempCarId', carId.value)
  ...
  isRouting.value = true
  try {
    await safeNavigateTo(buildCouponNavigateParams({ carId: carId.value, isPayment: isPayment.value, defaultTab: 'combo' }))
  } finally { setTimeout(() => { isRouting.value = false }, 300) }
}
...
async function onConfirm(){
  if (!agree.value) { uni.showToast({ title:'请先勾选协议', icon:'none' }); return }
  openPopup()
}
...
async function ConfirmOK(){
  try{
    if (!isPayment.value && !agree.value) {
      uni.showToast({ title:'请先勾选协议', icon:'none' }); 
      return
    }
    if (!carId.value) {
      uni.showToast({ title:'缺少车辆ID', icon:'none' }); 
      return
    }
    if (!returnPoint.value?.id) { uni.showToast({ title:'请选择还车点', icon:'none' }); return }
    openPopup()
    ...
    if (dynamicDispatchFee.value > 0) {
      orderDetails.push({
        name: '异地还车调度费',
        price: Number(dynamicDispatchFee.value),
        quantity: 1,
        unit: '项',
        businessType: 'dispatchFee',
        type: 'shortRent',
        remark: '还车点与取车点不同产生的调度费'
      })
    }
    const rp = returnPoint.value || {}
    const payload = {
      userId: toLongOrNull(userId.value),
      carId:  toLongOrNull(carId.value),
      comboId: toLongOrNull(selectedPackage.value || selectedCombo.value?.comboId),
      couponId: toLongOrNull(selectedCouponId.value ?? selectedCoupon.value?.couponId),
      orderType: 'shortRent',
      shouldTakeCarTime: takeTime,
      shouldReturnCarTime: returnTime,
      returnNetPointId:   toLongOrNull(rp.id),
      returnNetPointName: rp.name || '',
      returnLatitude:     (typeof rp.latitude  === 'number' ? rp.latitude  : null),
      returnLongitude:    (typeof rp.longitude === 'number' ? rp.longitude : null),
      dispatchFee: Number(dynamicDispatchFee.value || 0),
    ...
    // 初始化“本单是否已做过首次开门拍照”的状态
    const k = `order:${String(orderNum)}:first-open-proof`
    hasFirstOpenProof.value = !!uni.getStorageSync(k)
...
async function handleConfirmReturn(){
  const orderNum = String(uni.getStorageSync('currentOrderNum') || '')
  if (!orderNum) { uni.showToast({ title:'缺少订单号', icon:'none' }); return }
  ...
  try {
    ...
    await safeNavigateTo(`/pkg-biz/pages/return/photos?orderNum=${orderNum}`)
  } finally {
    setTimeout(() => { isRouting.value = false }, 300)
  }
}

async function onReturnSubmit(photoUrls){
  // TODO: 上传 photoUrls 到后端
  if (RETURN_PROOF_KEY.value){
    uni.setStorageSync(RETURN_PROOF_KEY.value, 1)
    hasReturnProof.value = true
  }
  showReturnPhoto.value = false
  // 拍照完成后，进入原有结算流（可按你的业务在照片页完成后再触发）
  // 这里保留原来入口：由“还车照片页”完成后回到此页再调用 settle
  try{
    isLoading.value = true
...
async function handlePay(){
  ...
  try{
    ...
    if (res?.success){
      ...
      setTimeout(async () => {
        await safeRedirectTo('/pages/home/home')
      }, 1200)
    }else{
...
async function doOpenDoor(){
  try{
    ...
    await openCar({ carId: carId.value })
    uni.hideLoading()
    uni.showToast({ title:'已开门', icon:'success' })
  }catch(e){
    uni.hideLoading()
...
const handleOpenDoor = () => {
  if (!ORDER_OPEN_PROOF_KEY.value) {
    doOpenDoor()
    return
  }
  if (!hasFirstOpenProof.value) {
    showPickupPhoto.value = true
  } else {
    doOpenDoor()
  }
}
...
const feeRowsUI = computed(() => {
  ...
  if (dynamicDispatchFee.value > 0) {
    rows.push({ key: 'dispatch', label: '调度费', price: dynamicDispatchFee.value })
  }
  return rows
})

// 取车拍照完成回调
async function onPickupSubmit(photoUrls){
  // TODO: 上传照片
  if (ORDER_OPEN_PROOF_KEY.value){
    uni.setStorageSync(ORDER_OPEN_PROOF_KEY.value, 1)
    hasFirstOpenProof.value = true
  }
...
/* 不在同一还车点的调度费 */
const dynamicDispatchFee = computed(() => {
  const rid = Number(returnPoint.value?.id || 0)
  const tid = Number(carInfo.value?.netPointId || 0)
  if (!rid || !tid) return 0
  return rid === tid ? 0 : 10
})
</script>
...
.car-card{ position:relative; left:20rpx; top:70rpx; width:650rpx; background:#fff; border-radius:36rpx; box-shadow:0 8rpx 20rpx rgba(0,0,0,.06); padding:20rpx 36rpx 24rpx 24rpx; overflow:hidden; }
...
.hero-spacer{ height:260rpx; }

/* 卡片通用 */
.card{ width:680rpx; margin:0rpx auto 0; background:#fff; border-radius:32rpx; }
...
.btn-pill{ min-width:128rpx; height:48rpx; padding:0 20rpx; border-radius:24rpx; font-size:28rpx; font-weight: 500; display:flex; align-items:center; justify-content:center; }
.pill-outline{ background:#fff; color:#FFBD37; border:2rpx solid #FFBD37; }
...
/* 费用明细行（左列定宽，右列不挤压） */
.fee-hd{ padding:24rpx;  margin-top:24rpx; padding-bottom:0; display:flex; align-items:center; gap:12rpx; }
...
.badge-gray{ padding:4rpx 12rpx;  border-radius:8rpx; font-size:24rpx; line-height:32rpx; color:#8A8A8A; white-space:nowrap; flex:0 0 auto; }
...
/* 若还车提示（强制一行 + 图标不塌陷） */
.elec{ display:flex; align-items:center; gap:12rpx; margin:18rpx 24rpx 24rpx; padding:12rpx 16rpx; background:transparent; border-radius:24rpx; }
...
.pkg-empty{
 ...
 background: linear-gradient( 90deg, #FFF3DF 0%, #FFD38A 100%);
  border-radius: 24rpx 24rpx 0rpx 0rpx;  /* 上边圆角，下边直角 */
}
...
.coupon-inner{ margin: 0 auto 32rpx;  width:calc(100% - 50rpx); height:128rpx; border-radius:24rpx; background:linear-gradient(90deg,#F66600 0%, #FD9A3D 100%); padding:0 24rpx; display:flex; align-items:center; justify-content:space-between; }
...
.coupon-empty{
  ...
  align-items: center;
  justify-content: space-between;
  background: linear-gradient(267deg, #F66600 0%, #FD9A3D 100%);
  border-radius: 24rpx;
}
