<template>
  <view class="page">
    ...
    <view class="hero">
      ...
      <!-- 位置搜索胶囊（点击跳转到还车/选网点页） -->
      <view class="search-pill" @click="goPickNet">
    ...
    <!-- 内容区 -->
    <scroll-view scroll-y class="content" :scroll-into-view="scrollTarget">
      <!-- 网点卡片（离你最近的网点） -->
      <view class="net-card" v-if="hasNets" id="nets-top">
      ...
      <view v-if="!hasNets" class="empty">暂无网点</view>
...
const hasNets = computed(() => netPoints.value.length > 0)
...
function scrollToNets(){
  scrollTarget.value = 'nets-top'
  setTimeout(() => (scrollTarget.value = ''), 300)
}
...
async function fetchCars(){
  ...
  try{
    ...
    const list = (res?.data || [])
    recommendedCars.value = list.sort((a,b)=> (b.aiWeight ?? 0) - (a.aiWeight ?? 0))
  }catch(e){
    console.error(e)
...
async function fetchNets(){
  ...
  try{
    ...
    if (netPoints.value.length && !currentNetId.value){
    ...
    console.error(e)
    uni.showToast({ title:'网点加载失败', icon:'none' })
  }finally{
...
onShow(() => {
  // 接收“还车/选网点页”返回的选择
  const picked = uni.getStorageSync('pickedNet')
  if (picked && picked.id){
    uni.removeStorageSync('pickedNet')
    selectNet(picked)
  }
})
</script>
...
.car-card{
  ...
  box-shadow:0 10rpx 28rpx rgba(0,0,0,.08); position: relative;
}
.model-chip{
  position:absolute;left:16rpx;top:16rpx;background:#e7fff1;color:#19bf6f;
...
.yen{font-size:28rpx;color:#ff2f2f}
.price-num{font-size:44rpx;color:#ff2f2f;font-weight:800}
.price-qi{font-size:26rpx;color:#ff2f2f}
.btn-green{
