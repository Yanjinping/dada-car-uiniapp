...
const functionItems = reactive([
  ...
  { title: '晒单好礼', icon: 'gift', onTap: 'goShareGift' },
  { title: '社群福利', icon: 'community', onTap: 'goCommunity' }
])
...
const NAV = {
  ...
  pay: '/pages/pay/pay',
  packages: '/pages/coupon/index?defaultTab=combo',
  coupons: '/pages/coupon/index?defaultTab=coupon',
  orders: '/pages/order/list',
  deposit: '/pages/wallet/deposit?tab=deposit',
  violation: '/pages/map/index?tab=violation', // 没有独立违章页时临时放 map
  auth: '/pages/auth/idcard',
  share: '/pages/order/ShareOrder',
  community: '/pages/promo/index',
...
const onCellTap = (it:any) => {
  switch(it?.onTap){
    ...
    case 'goShareGift':   safeGo(NAV.share); break
    case 'goCommunity':   safeGo(NAV.community); break
