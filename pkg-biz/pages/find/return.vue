...
<template>
  <view class="page">
    ...
    <view class="hero">
      <!-- 自定义标题栏（透明） -->
      <TitleBar title="还车点" background="transparent" />
...
import { getCurrentLocation, readGlobalCache, openLocationSetting, chooseLocation } from '@/utils/location'
...
onLoad((options) => {
  // 从 URL 读 carId（下单页已经拼到了 URL）
  currentCarId = Number(options?.carId || uni.getStorageSync(STORAGE_CAR_ID) || 0)
  // 保险：落一份当前车 ID，供其它页 onShow 读取
  uni.setStorageSync(STORAGE_CAR_ID, currentCarId)
...
async function fetchList(reset = false) {
  ...
  try {
    ...
    if (!userLoc.value) {
      userLoc.value = await getCurrentLocation() // 允许 null（无定位模式）
    }
...
function onChoose(item) {	
 ...
 // ✅ 正确的写入结构
   uni.setStorageSync(STORAGE_RP, { carId: Number(currentCarId), rp })
   // 兼容：把老 key 清掉，避免干扰
   uni.removeStorageSync('returnPoint')
  uni.navigateBack()
}
